...
void VoxelMapManager::StateEstimation(StatesGroup &state_propagat)
{
  ...
  for (int iterCount = 0; iterCount < config_setting_.max_iterations_; iterCount++)
  {
    ...
    effct_feat_num_ = ptpl_list_.size();
        fastlivo_cout() << "[ LIO ] Raw feature num: " << feats_undistort_->size() << ", downsampled feature num:" << feats_down_size_ 
...
// 对于每个点：
// 把点从世界坐标系映射到体素索引（voxel grid index）。
// 在 voxel_map_ 中查找对应体素（VOXEL_LOCATION）。
// 如果体素已存在，则把点更新到该体素内部的八叉树（VoxelOctoTree）；
// 否则创建一个新的 VoxelOctoTree，设置初始参数后再把点插入。
void VoxelMapManager::UpdateVoxelMap(const std::vector<pointWithVar> &input_points)
{
  float voxel_size = config_setting_.max_voxel_size_;//体素尺寸
  float planer_threshold = config_setting_.planner_threshold_;//平面判断阈值
  int max_layer = config_setting_.max_layer_;//八叉树最大层数
  int max_points_num = config_setting_.max_points_num_;//单体素/节点允许的最大点数
  std::vector<int> layer_init_num = convertToIntVectorSafe(config_setting_.layer_init_num_);
  uint plsize = input_points.size();//输入点数量
  for (uint i = 0; i < plsize; i++)
  {
    const pointWithVar p_v = input_points[i];
    float loc_xyz[3]; 
    ...
    {
      loc_xyz[j] = p_v.point_w[j] / voxel_size;
      if (loc_xyz[j] < 0) { loc_xyz[j] -= 1.0; }//这是为了解决 C/C++ 的强制类型转换对负数是向 0 截断（trunc），而不是向下取整（floor）的行为。
    }
    VOXEL_LOCATION position((int64_t)loc_xyz[0], (int64_t)loc_xyz[1], (int64_t)loc_xyz[2]);
    auto iter = voxel_map_.find(position);
