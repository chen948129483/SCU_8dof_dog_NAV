...
def generate_launch_description():
    ...
    return LaunchDescription([
        ...
        avia_config_arg,
        camera_config_arg,
        DeclareLaunchArgument('log_level', default_value='WARN', description='Log level for fastlivo_mapping'),
        use_respawn_arg,
        ...
        Node(
            ...
            # pass ros args to set runtime log level
            arguments=['--ros-args', '--log-level', log_level],
            respawn=use_respawn,
            remappings=[('/aft_mapped_to_init', '/state_estimation')],
            prefix=[
                # Uncomment to run under gdb or valgrind for debugging
                # ("gdb -ex run --args"),
                # ("valgrind --log-file=./valgrind_report.log --tool=memcheck --leak-check=full --show-leak-kinds=all --track-origins=yes --show-reachable=yes --undef-value-errors=yes --track-fds=yes")
            ],
            output="screen"
        ),
