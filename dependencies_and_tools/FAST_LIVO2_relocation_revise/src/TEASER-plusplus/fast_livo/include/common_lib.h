...
// Global debug enable flag. When false, legacy printf/cout debug output is suppressed.
extern bool g_fastlivo_enable_debug;
// Backwards-compatible quiet-console flag (true = suppress legacy prints).
extern bool g_fastlivo_quiet_console;

// Replacement for printf that respects the global debug flag.
#include <cstdarg>
#include <cstdio>
static inline int fastlivo_printf(const char *fmt, ...)
{
  if (!g_fastlivo_enable_debug) return 0;
  va_list args; va_start(args, fmt); int r = vprintf(fmt, args); va_end(args); return r;
}

// Redirect plain printf(...) calls used throughout the codebase to the guarded version.
#define printf(...) fastlivo_printf(__VA_ARGS__)

// Provide a safe conditional ostream for debug streaming.
// Avoid redefining the identifier `cout` which breaks `std::cout` occurrences.
class FastLivoNullBuf : public std::streambuf {
public:
  int overflow(int c) override { return c; }
};
static FastLivoNullBuf fastlivo_nullbuf;
static std::ostream fastlivo_nullstream(&fastlivo_nullbuf);

static inline std::ostream &fastlivo_cout()
{
  return g_fastlivo_enable_debug ? std::cout : fastlivo_nullstream;
}
...
//封装一帧激光相关数据（原始点云、处理缓冲、内含的 MeasureGroup 
//队列等），构造函数为指针成员分配了空点云对象并重置时间/标志。
