...
/*『GT』*/
#include <pcl_conversions/pcl_conversions.h> //preprocess.h中已经包含该头文件，这里重复包含不会有问题
#include <pcl/point_cloud.h> //type.h中已经包含该头文件，这里重复包含不会有问题 
#include <pcl/point_types.h> //type.h中已经包含该头文件，这里重复包含不会有问题
#include <pcl/filters/voxel_grid.h> //vio.h中已经包含该头文件，这里重复包含不会有问题
...
// #include <tf/transform_datatypes.h>  // tf::createQuaternionMsgFromRollPitchYaw (若使用 tf API)
// #include "type.h"                    // 项目中 PointType / PointCloudXYZI typedef（若存在，确保路径正确）
/*『GT』*/
...
public:
  ...
  double filter_size_surf_min = 0; //体素滤波器的叶子大小，应该可以自己设置
  double filter_size_map_min = 0; //『GT』
  ...
  // bool send_odom_base_tf = false;//是否发布odom到base的tf变换,当前用不上
  ...
  PointCloudXYZI::Ptr feats_down_prior_map; //『GT』
  pcl::PointCloud<PointType>::Ptr prior_map;//『GT』
  ...
  std::string prior_map_path;//『GT』
  rclcpp::Subscription<sensor_msgs::msg::Imu>::SharedPtr sub_imu;
  rclcpp::Subscription<sensor_msgs::msg::Image>::SharedPtr sub_img;
  rclcpp::Subscription<geometry_msgs::msg::PoseWithCovarianceStamped>::SharedPtr sub_init_pose_;//『GT』
  ...
  std::string initial_frame_id_ = "camera_init"; // frame for published poses/pointclouds
  std::string aft_mapped_frame_ = "aft_mapped";  // child frame for aft_mapped odometry
  // debug flags
  bool print_imu_debug_ = false;
