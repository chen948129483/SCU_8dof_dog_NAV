...
{
  BackUpTwzFree::BackUpTwzFree()
    : command_time_allowance_(rclcpp::Duration::from_seconds(0.0))  // 初始化 Duration 成员（0秒）
  {
  }
  ...
  void BackUpTwzFree::onConfigure()
  {
    ...
    node->declare_parameter("robot_radius", rclcpp::ParameterValue(0.1));
    node->declare_parameter("max_radius", rclcpp::ParameterValue(1.0));
    node->declare_parameter("service_name", rclcpp::ParameterValue(std::string("local_costmap/get_costmap")));
    node->declare_parameter("free_threshold", rclcpp::ParameterValue(5));
    node->declare_parameter("visualization", rclcpp::ParameterValue(false));
